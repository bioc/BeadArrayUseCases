...
Usually the directory will be in a known location, but for convenience we use the \Rfunction{system.file} function this directory and the sample sheet. The section names to be read are then constructed using the contents of the sample sheet.
...
\begin{exc}
Retrieve the Illumina Humanv2 IDs that are involved in positive regulation of the cell cycle according to GO (GO:0045787)
or KEGG (04110)
